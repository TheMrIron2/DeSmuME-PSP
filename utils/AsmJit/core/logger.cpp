...
//HCF Jit
size_t Logger::strnlenHCF(const char *s, size_t n)
{
...
void Logger::setInstructionPrefix(const char* prefix)
{
  ...
  if (!prefix)
	...
	//HCF strnlen
	size_t length = strnlenHCF(prefix, ASMJIT_ARRAY_SIZE(_instructionPrefix) - 1);

//  size_t length = strnlen(prefix, ASMJIT_ARRAY_SIZE(_instructionPrefix) - 1);
/*
  size_t length = strlen(prefix);
  if(length > ASMJIT_ARRAY_SIZE(_instructionPrefix) - 1) 
      length = ASMJIT_ARRAY_SIZE(_instructionPrefix) - 1;
*/  
