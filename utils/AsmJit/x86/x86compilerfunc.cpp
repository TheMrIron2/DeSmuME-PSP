...
void X86CompilerFuncDecl::_preparePrologEpilog(CompilerContext& cc)
{
  ...
  {
    int32_t v = 16 - sizeof(unsigned long);
...
void X86CompilerFuncCall::_moveAllocatedVariableToStack(CompilerContext& cc, X86CompilerVar* vdata, const FuncArg& argType)
{
  ...
  Mem dst = ptr(zsp, -(int)sizeof(unsigned long) + argType.getStackOffset());
  ...
  switch (vdata->getType())
  {
    case kX86VarTypeGpd:
      switch (argType.getVarType())
      {
		...
		//HCF SUICIDEEEEE Adding this didn't work
		/***
		default:
		x86Compiler->emit(kX86InstMov, dst, gpd(src));
          return;
...
/***
FILE *fdlu;
fdlu = fopen("zzlog-7.txt","a");
fprintf(fdlu,"URK: _moveSpilledVariableToStack\n");
fclose(fdlu);
***/
...
FILE *fdlu;
fdlu = fopen("zzlog-7.txt","a");
fprintf(fdlu,"URK: _moveSrcVariableToRegister\n");
fclose(fdlu);
***/
