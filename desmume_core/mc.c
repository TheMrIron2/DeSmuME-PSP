...
const char Gamesav[256];
//strncpy(Gamesav,filename,strlen(filename));
...
void mc_reset_com(memory_chip_t *mc)
{
   ...
   if (mc->type == MC_TYPE_AUTODETECT && mc->com == BM_CMD_AUTODETECT)
   {
	 ...
	 // 64 Kbit EEPROM
      ...
      // Generate file
      if ((mc->fp = fopen(Gamesav, "wb+")) != NULL)
...
void mc_load_file(memory_chip_t *mc, const char* filename)
{
   ...
   if(file == NULL)
   {
      strncpy(Gamesav,filename,strlen(filename));//mc->filename = strdup(filename);
      return;
   }
