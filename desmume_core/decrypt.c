...
static void decrypt_arm9(u32 cardheader_gamecode, unsigned char *data)
{
	...
	pspDebugScreenPrintf("Decryption failed!\n");
...
int DetectRomType(char* romdata)
{
	int i;
    enc_header* header = malloc(sizeof(enc_header));	
...
void DecryptSecureArea(u8 *romdata, long romlen)
{
	...
	enc_header* header = malloc(sizeof(enc_header));
	header = (enc_header*)romdata;

	romType = DetectRomType((char*)romdata);
	...
	pspDebugScreenSetTextColor(0xffffffff);
	pspDebugScreenSetXY(0,1);
	pspDebugScreenPrintf("Rom Already decrypted.\n");
	sceKernelDelayThread(100000);
	pspDebugScreenClear(); 
#endif  
	}
	...
	pspDebugScreenSetTextColor(0xffffffff);
	pspDebugScreenSetXY(0,1);
	pspDebugScreenPrintf("Rom Decrypted with success. \n");
	sceKernelDelayThread(100000);
	pspDebugScreenClear();
#endif
  	}
	...
	pspDebugScreenSetTextColor(0xffffffff);
	pspDebugScreenSetXY(0,1);
	pspDebugScreenPrintf("File doesn't appear to have a secure area.\n");
	sceKernelDelayThread(100000);
	pspDebugScreenClear(); 
#endif
	}
