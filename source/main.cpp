...
//HCF This is used for AUTO frameskip
//HCF We want 60 FPS, so it should be 16,6 milliseconds!!!
////#define EXPECTED_FRAME_TIME 17
#define MAX_EXPECTED_FRAME_TIME 17  //o 16???   //Estos no -> //18   //o 19?
...
{
    exit(0);
    //XLaunchNewImage( "D:\\NDSX.xbe", NULL);
}
...
void Gu_draw()
{
	...
	//HCF Toggle video mode
    /*
	if( iModoGrafico != iModoGraficoNuevo )
	{
		iModoGrafico = iModoGraficoNuevo;
		iModoStretch = iModoStretchNuevo;
		switch(iModoGrafico)
		{
			case 1:
				rectPant1.x = PANTALLA2_UNICA_X;
				rectPant1.y = PANTALLA2_UNICA_Y;
				SDL_ShowCursor(SDL_DISABLE);
				SDLscreen = SDL_SetVideoMode(320, 200, 16, SDL_HWPALETTE | SDL_RESIZABLE);
				break;
			case 2:
				rectPant2.x = PANTALLA2_UNICA_X;
				rectPant2.y = PANTALLA2_UNICA_Y;
			...
			default:
				...
				//HCF This is updated later
				//rectPant1.x = 128;
				rectPant1.y = 0;
				rectPant2.x = PANTALLA2_X;
				rectPant2.y = PANTALLA2_Y;
				SDLscreen = SDL_SetVideoMode(512, 384, 16, SDL_HWPALETTE | SDL_RESIZABLE);

				//HCF Trying to optimize-A (needed because we only fill the middle of the screen below)
	...
	else if( iModoStretch != iModoStretchNuevo )
	{
		iModoStretch = iModoStretchNuevo;

		//HCF Trying to optimize-A (needed because we only fill the middle of the screen below)
		if( iModoStretchNuevo == STRETCH_MODE_NONE )
		{
			SDL_FillRect(SDLscreen, NULL, SDL_MapRGB(SDLscreen->format, 0, 0, 0));
		}
	}
    */
    ...
    rectPant1.x = 0;
    rectPant1.y = 0;
	rectPant2.x = 223;   //CAMBIAR ESTO!!! OJO!!!
	rectPant2.y = 0;
...
//int main(int argc, char ** argv) {
  ...
  */
  ...
  if ( !my_config.disable_sound) {
	  //SPU_ChangeSoundCore(SNDCORE_DIRECTX, 735 * 4);
  ...
  iModoVideo = MODO_VIDEO_480;
  FILE *fdvideo;
  fdvideo = fopen(FILE_VIDEO, "r");
  if( fdvideo != NULL )
  {
    	fscanf( fdvideo, "%d", &iModoVideo);
		fclose(fdvideo);
  }
  */
  ...
  inIdiomaFirmware = 1;
  FILE *fdsettings;
  fdsettings = fopen(FILE_SETTINGS, "r");
  if( fdsettings != NULL )
  {
    	fscanf( fdsettings, "%d", &inIdiomaFirmware);
		fclose(fdsettings);
  }
  */
  ...
  fdini = fopen(FILE_DATA_INI, "r");
	...
	{
		jjj = 0;
		while(!feof(fdini) && ((chCar=fgetc(fdini)) != '\n' ) )
		{
			dataParams[jjj] = chCar;
			jjj++;
		}
		dataParams[jjj] = '\0';
		fclose(fdini);
	...
	else
	{
		...
		//XLaunchNewImage( "D:\\default-JIT.xbe", NULL);
		XLaunchNewImage( "D:\\default.xbe", NULL);
	}
	*/
	
	//HCF Parametros usados en NDS
...
#define LIMIT_FRAMERATE_DESMUME  2
  ...
  //HCF con la que eligio el usuario
  ...
  */
  ...
  vdDejaLog("SET VIDEO MODE");
    ...
    //HCF PSP
    //SDLscreen = SDL_SetVideoMode(480, 272, 16, SDL_HWPALETTE | SDL_RESIZABLE);
