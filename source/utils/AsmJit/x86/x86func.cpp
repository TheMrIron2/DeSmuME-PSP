...
//HCF LOGS
//Aqui ya llega mal el argumentsData de algunos
/*
FILE * fdlom;
fdlom = fopen("zzlog-7.txt","a");
fprintf(fdlom,"initDef, argumentsData[%d] es %d y convertido %d\n",i,argumentsData[i],static_cast<uint8_t>(argumentsData[i]));
fclose(fdlom);
*/
...
//#if defined(ASMJIT_X86)
  ...
  for (i = 0; i < (int32_t)argumentsCount; i++)
  {
    ...
    if (!arg.hasRegIndex())
    {
      arg._stackOffset += static_cast<uint16_t>(static_cast<int32_t>(sizeof(unsigned long)) - stackOffset);
    }
  }
