  ...
  else if (op->isMem())
  {
    ...
    *buf++ = '[';
    ...
    if (mem.getDisplacement() && !isAbsolute)
    {
      ...
      buf = StringUtil::utoa(buf, static_cast<unsigned long>(d), base);
    }
...
//HCF Jit
static size_t MystrnlenHCF(const char *s, size_t n)
{
...
static char* X86Assembler_dumpComment(char* buf, size_t len, const uint8_t* binaryData, size_t binaryLen, const char* comment)
{
  ...
  //HCF strnlen
  size_t commentLength = comment ? MystrnlenHCF(comment, kMaxCommentLength) : 0;
  ...
  if(comment)
	  ...
	  if(commentLength > kMaxCommentLength) 
		  commentLength = kMaxCommentLength; 
  else
      commentLength = 0;
  */
  //HCF strnlen end
...
#define _FINISHED_IMMEDIATE(_Operand_, _Size_) \
  ...
  switch (id->getGroup())
  {
    ...
    case kX86InstGroupEnter:
    {
      ...
      {
        _emitByte(0xC8);
        _emitWord((uint16_t)(unsigned long)reinterpret_cast<const Imm&>(*o2).getValue());
        _emitByte((uint8_t )(unsigned long)reinterpret_cast<const Imm&>(*o1).getValue());
        _FINISHED();
      }
      break;
    }
