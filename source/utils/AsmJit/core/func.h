...
#define ASMJIT_DECLARE_TYPE_CORE(_PtrId_) \
  ...
  { \
    enum \
    { \
      Id = (sizeof( ::TypeId_NoPtiHelper((T(*)())0) ) == sizeof(char) \
        ? static_cast<int>(_PtrId_) \
        : static_cast<int>(::kVarTypeInvalid)) \
    }; \
  }
...
struct FuncBuilder2 : public FuncPrototype
{
  inline FuncBuilder2()
  {
	...
	char b[50];
	char c[50];
	strcpy(a,typeid(RET).name());
	strcpy(b,typeid(RET).name());
	strcpy(c,typeid(RET).name());
	***/
