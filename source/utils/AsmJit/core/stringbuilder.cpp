...
char* StringBuilder::prepare(uint32_t op, size_t len)
{
  ...
  if (op == kStringBuilderOpSet)
  {
    ...
    if (_capacity < len)
    {
      if (len >= IntUtil::maxValue<size_t>() - sizeof(unsigned long) * 2)
        return NULL;

      size_t to = IntUtil::align<size_t>(len, sizeof(unsigned long));
      if (to < 256 - sizeof(unsigned long))
        to = 256 - sizeof(unsigned long);

      char* newData = static_cast<char*>(ASMJIT_MALLOC(to + sizeof(unsigned long)));
      if (newData == NULL)
      {
        clear();
        return NULL;
      ...
      _data = newData;
      _capacity = to + sizeof(unsigned long) - 1;
      _canFree = true;
    }
  ...
  else
  {
    ...
    // Overflow.
    if (IntUtil::maxValue<size_t>() - sizeof(unsigned long) * 2 - _length < len)
      return NULL;
    ...
    if (_capacity < after)
    {
      ...
      to = IntUtil::align<size_t>(to, sizeof(unsigned long));
      char* newData = static_cast<char*>(ASMJIT_MALLOC(to + sizeof(unsigned long)));
...
bool StringBuilder::reserve(size_t to)
{
  ...
  if (to >= IntUtil::maxValue<size_t>() - sizeof(unsigned long) * 2)
    return false;
