...
struct Buffer
{
  ...
  //! @brief Emit unsigned long (4 or 8 bytes).
  inline void emitUIntPtrT(unsigned long x)
  {
    ASMJIT_ASSERT(getOffset() + sizeof(unsigned long) <= _capacity);

    *(unsigned long *)_cur = x;
    _cur += sizeof(unsigned long);
  }
  ...
  //! @brief Set unsigned long at position @a pos.
  inline unsigned long getUIntPtrTAt(size_t pos) const
  {
    ASMJIT_ASSERT(pos + sizeof(unsigned long) <= _capacity);

    return *reinterpret_cast<const unsigned long*>(_data + pos);
  }

  //! @brief Set size_t at position @a pos.
  inline unsigned long getSizeTAt(size_t pos) const
  {
  ...
  //! @brief Set unsigned long at position @a pos.
  inline void setUIntPtrTAt(size_t pos, unsigned long x)
  {
    ASMJIT_ASSERT(pos + sizeof(unsigned long) <= _capacity);

    *reinterpret_cast<unsigned long*>(_data + pos) = x;
  }
