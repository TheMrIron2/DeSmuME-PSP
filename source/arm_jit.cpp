...
static u32 compile_basicblock()
{
	...
	for(u32 i=0, bEndBlock = 0; bEndBlock == 0; i++)
	{
		...
		bEndBlock = instr_is_branch(opcode) || (i >= (100 - 1));
...
void arm_jit_reset(bool enable, bool suppress_msg)
{
	...
	*/
	saveBlockSizeJIT = 100;  //CommonSettings.jit_max_block_size;
	...
	if (enable)
	{
		...
		#define JITFREE(x)  for(int iii=0;iii<ARRAY_SIZE(x);iii++) if(x[iii]) MemoryManager::getGlobal()->free((void*)x[iii]);  memset(x,0,sizeof(x));
