...
FORCEINLINE edge_fx_fl::edge_fx_fl(int Top, int Bottom, VERT** verts, bool& failure) {
	...
	if(Height != 0 || Width != 0)
	{
		...
		if (dN != 0)
		{
			const long dN_mul = dN << 4;
			const long dM_mul = dM << 4;

			long InitialNumerator = (long)(dM_mul *Y - dM*verts[Top]->y + dN*verts[Top]->x - 1 + dN_mul);
			FloorDivMod(InitialNumerator, dN_mul,X,ErrorTerm,failure);
			FloorDivMod(dM_mul, dN_mul,XStep,Numerator,failure);
			Denominator = dN_mul;
		}
		else
		{
		...
		float YPrestep = Fixed28_4ToFloat(( (((fixed28_4)Y)<<4) - verts[Top]->y));
		float XPrestep = Fixed28_4ToFloat(( (((fixed28_4)X)<<4) - verts[Top]->x));

		float dy = sceFpuDiv(1,Fixed28_4ToFloat(dN));
		float dx = sceFpuDiv(1,Fixed28_4ToFloat(dM));
...
public:
	...
	FORCEINLINE FragmentColor sample(float u, float v)
	{
		...
		if(!sampler.enabled || my_config.Dspr3D) return white;
	...
	FORCEINLINE void shade(FragmentColor& dst)
	{
		...
		case 0: //modulate
			...
			break;
		case 1: //decal
			if(sampler.enabled && !my_config.Dspr3D)
			{
				u = shader.invu*shader.w;
				v = shader.invv*shader.w;
				texColor = sample(u,v);
